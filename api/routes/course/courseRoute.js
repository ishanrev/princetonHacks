  ...
  for (let module of baseSchemaJSON.modules) {
    ...
    for (let video of module.videos) {
      ...
      try {
        ...
        let summary = await createSummary(`give me a summary of the video, if you are unable to give me a summary of the video, provide me random summary based on the topic ${module.title}`)
...
router.post('/deploy', async (req, res) => {
  ...
  for (let module of course.modules) {
    ...
    try {
      ...
      newCourse.total = newCourse.total + module.videos.length + module.test.questions.length
      moduleNum++;
    } catch (saveError) {
...
router.post('/progress', async (req, res) => {
  ...
  console.log(user)
  let temp = user.courses;
  temp[courseId].progress += 1;

  let tempCoursesCompleted = user.coursesCompleted
  if (temp[courseId].progres == course.total) {
    tempCoursesCompleted[courseId] = true;
  }
  await users.findByIdAndUpdate(userId, { courses: temp, coursesCompleted: tempCoursesCompleted })
  // await user.save()
